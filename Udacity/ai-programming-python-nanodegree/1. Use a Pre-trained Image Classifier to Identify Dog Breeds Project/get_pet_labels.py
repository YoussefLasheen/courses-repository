...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(in_files), 1):
       ...
       if in_files[idx][0] != ".":
           ...
           removedSuffix = ''.join(i for i in in_files[idx].split(".")[0])
           removedUnderscores = ''.join(m for m in removedSuffix.replace("_"," "))
           removedNumbers = ''.join([x for x in removedUnderscores if not x.isdigit()])
           
           pet_label = removedNumbers.lower().strip()
